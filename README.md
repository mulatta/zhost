...
>
> See [`ROADMAP.md`](ROADMAP.md) for the API-first multi-user release plan and
> [`TECHNICAL_DEBT.md`](TECHNICAL_DEBT.md) for the prioritized implementation
> debt. The current server remains single-user while that pre-alpha work lands.
...
by a bearer token and must be exposed only over HTTPS; the one path that hands
out a token — the login session's `/login` step — must be gated, either by SSO
(public deployment) or by a private network (WireGuard/ZeroTier). The current
token is provisioned as a secret, never generated by the server. This pre-alpha
implementation has not received a security audit.
...
Current implementation includes the pre-alpha multi-user foundation: identity
mapping, scoped API keys, personal and group libraries, memberships, and
library-scoped metadata/file operations. Remaining gaps include full public API
inventory coverage, owner/admin/member hardening, GUI compatibility closure,
streaming, binary-diff uploads, backup/restore drills, and styled
bibliography/citation rendering. Internal Zotero.org infrastructure remains out
of scope.

## Development

```sh
nix develop            # postgres + sqlx-cli + rust toolchain + clippy
cargo fmt
